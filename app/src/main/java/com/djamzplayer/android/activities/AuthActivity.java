...
import com.djamzplayer.android.fragments.LoginFragment;
import com.djamzplayer.android.fragments.SignUpFragment;
import com.djamzplayer.android.models.User;
...
public  class AuthActivity extends AppCompatActivity implements LoginFragment.OnLoginFragmentListener, SignUpFragment.OnSignUpFragmentListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(com.djamzplayer.android.R.layout.activity_auth);
        sharedPreferences = getSharedPreferences(LOGIN_DATA, Context.MODE_PRIVATE);
        putDefaults();
        root =  findViewById( com.djamzplayer.android.R.id.auth_root);
        if(sharedPreferences.getBoolean(LOGGED_IN, false)){
            //logged in
            startMainActivity();
        }else{
            getSupportFragmentManager().beginTransaction()
                    .add(com.djamzplayer.android.R.id.auth_root, LoginFragment.newInstance())
    ...
    public void onBackPressed() {
        if (getSupportFragmentManager().getBackStackEntryCount() > 0) {
            ...
            Toast.makeText(this, com.djamzplayer.android.R.string.press_to_exit, Toast.LENGTH_SHORT).show();
    ...
    public void onSignUpBottomClick() {
        getSupportFragmentManager().beginTransaction()
                .replace(com.djamzplayer.android.R.id.auth_root, SignUpFragment.newInstance())
                .setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE)
                .addToBackStack(null)
                .commit();
    }
